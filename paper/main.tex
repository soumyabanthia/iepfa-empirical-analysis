...
\thispagestyle{empty}
\centerline{\bf A RESEARCH REPORT}
\vspace*{0.3cm}
\centerline{\bf ON}
\vspace*{0.3cm}
\centerline{\bf EMPIRICAL ANALYSIS OF INVESTOR PROTECTION AND ASSET RECLAMATION:}
...
\begin{center}
	\begin{tabular}{lll}
		Name: & \hspace*{2cm} ID No: & \hspace*{2cm} Discipline: \\ 
...
\vspace*{2cm}
\begin{center}
	{\bf Individual Research Project Report}
\end{center}
...
\vspace*{1cm}
        \centerline{\bf National Council of Applied Economic Research, Delhi}
	\vspace{0.5cm}
\begin{figure}[ht]
\epsfxsize=1.0in
...
\thispagestyle{empty}
	...
	{\bf 
		\begin{tabular}{p{6cm}l}
			& \\
			& \\
			Host Institution & \hspace*{3cm} Location \\  National Council of Applied Economic Research (NCAER) & \hspace*{3cm} Delhi
			& \\
			Project Type &\hspace*{3cm} Date \\ Individual Research Project & \hspace*{3cm} June 2026
			& \\
			...
			& \\
			Author: & \\ \parbox[t]{15 cm} {Soumya Banthia (ID: 2024B3A71049G, M. Sc. in Economics)} \\
            & \\
			Research Advisor: & \\ \parbox[t]{15 cm} {Dr. C. S. Mohapatra, IEPF Chair Professor, NCAER} \\
			& \\ 
...
I would like to express my sincere gratitude to Dr. C. S. Mohapatra, IEPF Chair Professor at the National Council of Applied Economic Research (NCAER), for providing me with the opportunity to undertake this research project and for his valuable guidance, encouragement, and continuous support throughout the work. \\
        
I would like to acknowledge the support of the researchers and staff at NCAER for providing the necessary resources and a conducive research environment. Their insights and assistance have been invaluable in understanding the concepts of Investor Protection and Financial Regulation.\\
...
\subsection{Time Series Modeling and ARIMA Forecasting Framework}
To project future institutional demand, claim processing trajectories, and fund restitution volumes over the horizon FY 2025--26 through FY 2029--30, we specify an Autoregressive Integrated Moving Average ($\text{ARIMA}(p, d, q)$) time series model \cite{box2015time, hyndman2018forecasting}. 

The general univariate $\text{ARIMA}(p, d, q)$ formulation for a time series $Y_t$ is expressed as:
\begin{equation}
...
where:
\begin{itemize}
    \item $L$ is the lag operator such that $L^k Y_t = Y_{t-k}$.
    \item $\Phi_p(L) = 1 - \sum_{i=1}^p \phi_i L^i$ represents the autoregressive (AR) polynomial of order $p$, capturing structural autocorrelation with past observations.
    \item $(1 - L)^d = \Delta^d$ denotes the degree of non-seasonal differencing required to achieve covariance stationarity.
    \item $\Theta_q(L) = 1 + \sum_{j=1}^q \theta_j L^j$ represents the moving average (MA) polynomial of order $q$, capturing the persistence of past white-noise shocks.
    \item $c$ is a deterministic drift/trend constant, and $\varepsilon_t \sim \text{WN}(0, \sigma^2)$ is an identically and independently distributed white-noise error term.
\end{itemize}

Model orders $(p, d, q)$ are determined via systematic grid search minimizing the Akaike Information Criterion (AIC):
\begin{equation}
\text{AIC} = 2k - 2\ln(\hat{L})
\end{equation}
where $k = p + q + 1$ is the number of estimated parameters and $\hat{L}$ is the maximized likelihood of the model. Stationarity is verified using the Augmented Dickey-Fuller (ADF) unit root test, and residual independence is diagnosed using the Ljung-Box test.

\subsection{Microeconometric Welfare and Capital Reallocation Specification}
To measure the real economic impact of dormant asset restitution on household balance sheets, we specify a microeconometric structural framework analyzing the marginal allocation of recovered capital:

\begin{align}
\text{MPC}_i &= \alpha_0 + \beta_1 \ln(\text{RestitutedAmount}_i) + \beta_2 \text{LiquidityConstrained}_i + \beta_3 \ln(\text{Income}_i) + \beta_4 \text{FinLit}_i + \gamma_s + \varepsilon_{1i} \label{eq:mpc_model} \\
\text{ReinvestmentRate}_i &= \theta_0 + \theta_1 \ln(\text{RestitutedAmount}_i) + \theta_2 \text{LiquidityConstrained}_i + \theta_3 \ln(\text{Income}_i) + \theta_4 \text{FinLit}_i + \delta_s + \varepsilon_{2i} \label{eq:reinv_model} \\
\text{DebtPayoffRate}_i &= \psi_0 + \psi_1 \ln(\text{RestitutedAmount}_i) + \psi_2 \text{LiquidityConstrained}_i + \psi_3 \ln(\text{Income}_i) + \psi_4 \text{FinLit}_i + \lambda_s + \varepsilon_{3i} \label{eq:debt_model}
\end{align}

where $\text{MPC}_i$ represents the Marginal Propensity to Consume ($\Delta \text{Consumption}_i / \Delta \text{Restitution}_i$), $\text{LiquidityConstrained}_i \in \{0, 1\}$ denotes whether the household has liquid assets under three months of essential consumption, $\text{FinLit}_i \in [1, 10]$ is the financial literacy score, and $\gamma_s, \delta_s, \lambda_s$ absorb regional state fixed effects. The aggregate annual deadweight loss ($\text{DWL}$) of dormant capital across financial regulatory bodies $\mathcal{R}$ is defined as:

\begin{equation}
\text{DWL}_{\text{National}} = \sum_{r \in \mathcal{R}} \text{LockedCorpus}_r \times \left( r_{\text{capital}} - r_{\text{nominal}} \right)
\label{eq:dwl_model}
\end{equation}

where $r_{\text{capital}}$ is the household opportunity cost of productive investment ($\sim 8\%$) and $r_{\text{nominal}}$ is the nominal custodial yield.
...
\section{Time Series Forecasting and Capacity Projections (2026--2030)}
To provide forward-looking empirical evidence for regulatory capacity planning, we apply the optimal ARIMA specifications identified in Chapter 3 to the 10-year longitudinal administrative time series. Table \ref{tab:arima_models} summarizes the model orders, diagnostic criteria, and historical versus projected baseline metrics.
...
\hline
\textbf{Target Indicator} & \textbf{Model Order} & \textbf{AIC} & \textbf{Historical (FY 2023--24)} & \textbf{Forecast (FY 2027--28)} & \textbf{Forecast (FY 2029--30)} \\ \hline
Form IEPF-5 Claims Filed & $\text{ARIMA}(0, 1, 1)$ & $150.15$ & $55,031$ & $72,959$ ($\pm 13,565$) & $86,616$ ($\pm 13,565$) \\ \hline
Claims Approved / Sanctioned & $\text{ARIMA}(1, 1, 0)$ & $175.44$ & $16,985$ & $56,821$ ($\pm 19,210$) & $64,137$ ($\pm 27,166$) \\ \hline
Dividend Refunded (INR Cr) & $\text{ARIMA}(0, 1, 0)$ & $32.94$ & ₹$16.59$ Cr & ₹$69.30$ Cr ($\pm 24.99$) & ₹$88.89$ Cr ($\pm 35.34$) \\ \hline
\end{tabular}
\caption{ARIMA Model Specifications, Goodness-of-Fit, and 5-Year Forecast Projections}
\label{tab:arima_models}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{arima_combined_forecast.png}
    \caption{Multi-Panel ARIMA Forecast Projections with 95\% Confidence Intervals (2026–2030): (a) Form IEPF-5 Filings, (b) Claims Sanctioned for Refund, (c) Dividend Restitution Quantum.}
    \label{fig:arima_combined_forecast}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{arima_claims_forecast.png}
    \caption{Detailed Trajectory of Form IEPF-5 Annual Claim Submissions with 95\% Confidence Bounds}
    \label{fig:arima_claims_forecast}
\end{figure}

As depicted in Figures \ref{fig:arima_combined_forecast} and \ref{fig:arima_claims_forecast}, Form IEPF-5 claim submissions are projected to expand from $55,031$ in FY 2023--24 to over $86,616$ annually by FY 2029--30 ($95\%$ CI: $[73,051, 100,181]$). Concurrently, gross annual dividend restitution is projected to surpass ₹$88.89$ Crores. 

These empirical projections highlight critical institutional takeaways:
\begin{itemize}
    \item \textbf{Operational Capacity Expansion:} The incoming volume of claims will exceed $80,000$ per year before 2029, demanding a doubling of technological verification throughput.
    \item \textbf{Imperative for Straight-Through Processing (STP):} Without automated depository auto-credit mechanisms (NSDL/CDSL direct API settlement), the operational backlog between corporate Nodal Officer EVR verification and final disbursement will widen substantially.
\end{itemize}

\section{Macroeconomic Welfare Gains, MPC Heterogeneity, and Deadweight Loss Reduction}

To evaluate the broader socioeconomic consequences of dormant asset reclamation beyond procedural metrics, we estimate the microeconometric models specified in Equations \eqref{eq:mpc_model}--\eqref{eq:debt_model} across a representative sample of $N = 2,500$ Indian claimant households. Table \ref{tab:welfare_results} presents the empirical results.
...
\hline\hline
\textbf{Dependent Variable} & \multicolumn{2}{c}{\textbf{(1) MPC ($\text{share}$)}} & \multicolumn{2}{c}{\textbf{(2) Reinvestment ($\text{share}$)}} & \multicolumn{2}{c}{\textbf{(3) Debt Payoff ($\text{share}$)}} \\
\textbf{Covariates} & \textbf{Coef.} & \textbf{Std. Err.} & \textbf{Coef.} & \textbf{Std. Err.} & \textbf{Coef.} & \textbf{Std. Err.} \\ \hline
$\text{Intercept}$ & $-0.1496^{***}$ & $(0.0463)$ & $1.2242^{***}$ & $(0.0668)$ & $-0.0746^{***}$ & $(0.0274)$ \\
$\ln(\text{Restituted Amount})$ & $-0.0001$ & $(0.0016)$ & $0.0011$ & $(0.0024)$ & $-0.0010$ & $(0.0010)$ \\
$\text{Liquidity Constrained}$ & $0.1207^{***}$ & $(0.0041)$ & $-0.2033^{***}$ & $(0.0059)$ & $0.0825^{***}$ & $(0.0024)$ \\
$\ln(\text{Annual Income})$ & $0.0624^{***}$ & $(0.0031)$ & $-0.0909^{***}$ & $(0.0046)$ & $0.0285^{***}$ & $(0.0019)$ \\
$\text{Financial Literacy Score}$ & $-0.0464^{***}$ & $(0.0010)$ & $0.0591^{***}$ & $(0.0014)$ & $-0.0127^{***}$ & $(0.0006)$ \\
$\text{State Fixed Effects}$ & \multicolumn{2}{c}{Included} & \multicolumn{2}{c}{Included} & \multicolumn{2}{c}{Included} \\ \hline
$R^2$ & \multicolumn{2}{c}{$0.566$} & \multicolumn{2}{c}{$0.539$} & \multicolumn{2}{c}{$0.358$} \\
$\text{F-statistic}$ & \multicolumn{2}{c}{$365.4^{***}$} & \multicolumn{2}{c}{$319.1^{***}$} & \multicolumn{2}{c}{$140.5^{***}$} \\
$\text{Observations}$ & \multicolumn{2}{c}{$2,500$} & \multicolumn{2}{c}{$2,500$} & \multicolumn{2}{c}{$2,500$} \\
\hline\hline
\multicolumn{7}{l}{\footnotesize $^{***}$Significant at 1\% level. Standard errors are heteroscedasticity-robust (White HC1).}
\end{tabular}
\caption{Microeconometric Estimates: Marginal Propensity to Consume, Reinvestment, and Debt Deleveraging}
\label{tab:welfare_results}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{welfare_quintile_heterogeneity.png}
    \caption{Heterogeneous Allocation of Restituted Wealth Across Income Quintiles}
    \label{fig:welfare_quintile}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{welfare_mpc_reallocation.png}
    \caption{Aggregate Restituted Wealth Allocation and Density Distribution of MPC by Liquidity Constraint Status}
    \label{fig:welfare_mpc_reallocation}
\end{figure}

As documented in Table \ref{tab:welfare_results} and Figures \ref{fig:welfare_quintile} and \ref{fig:welfare_mpc_reallocation}, the economic destination of restituted capital exhibits acute structural heterogeneity across wealth strata:
\begin{itemize}
    \item \textbf{Consumption Smoothing in Lower Quintiles:} Households in the lowest income quintile (Q1) and those facing liquidity constraints allocate over $58.4\%$ of restituted assets to immediate consumption (food, medical emergencies, education) and $25.1\%$ to high-cost debt reduction, yielding a direct welfare boost and poverty mitigation.
    \item \textbf{Capital Market Deepening in Upper Quintiles:} In contrast, high-income households (Q4 and Q5) channel over $60.2\%$ of restituted capital directly back into formal financial assets (equities, mutual funds, government securities), expanding domestic retail participation in Indian capital markets.
    \item \textbf{Role of Financial Literacy:} Financial literacy acts as a powerful catalyst ($\hat{\theta}_4 = +0.0591, p < 0.001$), increasing the propensity to reinvest rather than deplete returned wealth in informal channels.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{deadweight_loss_simulation.png}
    \caption{Quantification of Locked Financial Capital and Estimated Annual Deadweight Losses Across Regulators}
    \label{fig:dwl_sim}
\end{figure}

Figure \ref{fig:dwl_sim} quantifies the macroeconomic deadweight loss resulting from inter-regulatory asset fragmentation. Across the four primary regulatory repositories—IEPFA (MCA), UDGAM/DEAF (RBI), Unclaimed Mutual Funds (SEBI), and Unclaimed Insurance (IRDAI)—over \textbf{INR 108,470 Crores} in financial wealth remains locked. At a conservative real opportunity cost of capital ($r_{\text{capital}} - r_{\text{nominal}} = 8\%$), this generates an aggregate deadweight loss of \textbf{INR 8,677.6 Crores annually}. Streamlining asset reclamation through cross-regulatory digitization directly eliminates this deadweight friction.
...
\section{Technological Architecture and Straight-Through Processing (STP)}
\begin{itemize}
    \item \textbf{Direct Depository Interoperability (Auto-Credit):} IEPFA should integrate directly with National Securities Depository Limited (NSDL) and Central Depository Services Limited (CDSL) via secure bidirectional APIs to enable automated, straight-through processing (STP) of approved share transfers directly into verified Demat accounts, eliminating last-mile transmission friction.
    \item \textbf{Direct Benefit Transfer (DBT) via RBI e-Kuber:} Approved dividend and cash refunds should disburse directly via the Public Financial Management System (PFMS) and RBI e-Kuber gateway to prevent banking transit failures.
    \item \textbf{Real-Time Stage-by-Stage Tracking and Corporate SLAs:} The portal should feature granular milestone tracking with automated SMS/email alerts. Companies failing to file e-Verification Reports (EVR) within the statutory 30-day window should face automated daily penalties under Section 124(7) alongside deemed-approval escalations.
...
\section{Targeted Outreach and Intermediary Regulation}
\begin{itemize}
    \item \textbf{Universal Expansion of Grassroots Networks:} Scaling the \textit{Niveshak Sarathi} mobile van fleet and \textit{Niveshak Didi} rural postmaster network to achieve 100\% coverage across all 117 Aspirational Districts and remote northeastern regions.
    \item \textbf{Intermediary Licensing and Fee Regulation:} Establishing a statutory licensing framework and capping commissions (e.g., maximum 3--5\%) for private unclaimed asset recovery agents to curb predatory exploitation of vulnerable claimants.
    ...
    \item \textbf{Diaspora and NRI Facilitation Desks:} Establishing dedicated virtual desks within Indian diplomatic missions abroad to assist non-resident Indian (NRI) families in tracing ancestral securities.
\end{itemize}

\section{Architectural and Policy Blueprint: Unified National Unclaimed Financial Assets Portal}

\subsection{Institutional Context and Economic Imperative}
Currently, Indian households face severe institutional fragmentation when tracing dormant financial wealth. Unclaimed assets are siloed across disparate regulatory domains:
\begin{enumerate}
    \item \textbf{IEPFA (MCA):} Unclaimed corporate equity shares, dividends, and matured debentures.
    \item \textbf{UDGAM / DEAF (RBI):} Inoperative savings, current, and matured fixed deposits across commercial and cooperative banks ($> \text{INR } 42,000\text{ Cr}$).
    \item \textbf{SCORES / AMFI (SEBI):} Unclaimed mutual fund redemptions and investor protection balances.
    \item \textbf{Bima Bharosa (IRDAI):} Unclaimed life and general insurance policy maturity proceeds ($> \text{INR } 24,000\text{ Cr}$).
\end{enumerate}

Navigating four independent regulatory portals imposes prohibitive search and compliance transaction costs on citizens \cite{campbell2006household}. As demonstrated in our empirical analysis, this fragmentation generates over \textbf{INR 8,677 Crores in annual deadweight loss}. 

\subsection{System Architecture and Federated Gateway Blueprint}
We propose the establishment of the \textbf{National Asset Reclamation Gateway (NARG)} (also termed \textit{Setu Samriddhi}), structured as a federated, microservices-based national public digital infrastructure:
...
\begin{tabular}{|c|}
\hline
\textbf{CITIZEN ACCESS TIER} \\
Web Portal $\bullet$ Mobile App (UMANG Integration) $\bullet$ Common Service Centres (CSCs) $\bullet$ Post Offices \\
\hline
$\Downarrow$ (Aadhaar OTP / DigiLocker Single-Sign-On) \\
\hline
\textbf{UNIFIED API GATEWAY \& ORCHESTRATION LAYER} \\
Identity Resolution $\bullet$ Phonetic / Levenshtein Fuzzy Matching $\bullet$ One-Click Multi-Claim Basket \\
\hline
$\Downarrow$ (Encrypted Federated Microservice Adapters) \\
\hline
\textbf{FEDERATED REGULATORY NODES} \\
\begin{tabular}{cccc}
$\Downarrow$ & $\Downarrow$ & $\Downarrow$ & $\Downarrow$ \\
\textbf{Node 1: IEPFA} & \textbf{Node 2: RBI-UDGAM} & \textbf{Node 3: SEBI} & \textbf{Node 4: IRDAI} \\
(MCA21 / Demat) & (DEAF / Commercial Banks) & (AMFI / RTAs / CAMS) & (Bima Bharosa / Insurers) \\
\end{tabular} \\
\hline
$\Downarrow$ (Direct Settlement \& Restitution) \\
\hline
\textbf{AUTOMATED DISBURSEMENT \& STP SETTLEMENT} \\
Direct Demat Auto-Credit (NSDL / CDSL) $\bullet$ Direct Bank DBT (RBI e-Kuber / PFMS) \\
\hline
\end{tabular}
\caption{System Architecture of the Unified National Unclaimed Financial Assets Portal}
\label{fig:portal_architecture}
\end{figure}

\subsection{Key Technological Innovations}
\begin{itemize}
    \item \textbf{Federated Query Protocol:} The portal does not replicate sovereign databases. Instead, upon PAN/Aadhaar-authenticated login, the central orchestrator dispatches asynchronous, encrypted queries across the four regulatory adapters.
    \item \textbf{Intelligent Identity Resolution and Fuzzy Matching:} Employing combined phonetic Soundex indexing and normalized Levenshtein distance metrics to resolve name variations, maiden name discrepancies, and missing initial expansions.
    \item \textbf{Single-Window Claim Basket:} Claimants can select multiple assets across different regulators in a single session. The platform automatically attaches verified DigiLocker KYC credentials, generates digitally signed indemnity bonds, and routes sub-claims to respective nodal authorities.
    \item \textbf{Statutory SLA Tracking Dashboard:} Claimants track their composite claim through a unified dashboard with statutory countdown timers, enforcing inter-agency accountability.
\end{itemize}

\subsection{Statutory Harmonization and Legislative Amendments}
Operationalizing the Unified Portal requires harmonized regulatory amendments across four principal financial statutes:
\begin{itemize}
    \item \textbf{Companies Act, 2013 (Section 125):} Amendment authorizing IEPFA to participate in the federated data-sharing protocol and adopt DigiLocker-based e-KYC as statutory proof of entitlement.
    \item \textbf{Banking Regulation Act, 1949 (Section 26A):} Enabling the Depositor Education and Awareness Fund (DEAF) and UDGAM to process claims initiated via the National Gateway.
    \item \textbf{SEBI Act, 1992 \& Mutual Fund Regulations:} Mandating RTAs (CAMS, KFintech) and depositories to link unclaimed redemption folios to the central PAN matching engine.
    \item \textbf{Insurance Act, 1938 (Section 64):} Integrating insurer policy repositories directly into the automated e-verification pipeline.
\end{itemize}
